...
from src.model import PlayerDetectionModel, PlayerCaptionModel
...
def runPlayerDetection(file_paths: List[str], runtime_config: str=None):
    ...
    model = PlayerDetectionModel(config["container"]["detection weights"], runtime_config=cfg)
...
def runPlayerCaption(file_paths: List[str], runtime_config: str=None):
    if runtime_config is None:
        cfg = config["model"]
    else:
        cfg = json.loads(runtime_config)
    ...
    detectionmodel = PlayerDetectionModel(config["container"]["detection weights"], runtime_config=cfg)
    captionmodel = PlayerCaptionModel(config["container"]["caption weights"], runtime_config=cfg)
    ...
    for fname in file_paths:
        ...
        img = img[:, :, ::-1]
         # get xmp data from image
        headline = extract_xmp_as_dict(fname).get('Headline', '')
        detectionmodel.set_headline(headline)
        frametags = detectionmodel.tag(img)

        player_prediction = getPlayerLabels(frametags['text'], )
